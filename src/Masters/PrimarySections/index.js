...
import PrimarySectionsAdd from './add';
import PrimarySectionsEdit from './edit';
import PrimarySectionsDetail from './detail';
...
function Index() {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('load-primary-sections', (event, data) => {
            setPrimarySections(data);
        });

        ipcRenderer.on('primary-section-deleted', (event, id) => {
            setPrimarySections((prevSections) => prevSections.filter(section => section.id !== id));
        });

        ipcRenderer.on('search-primary-sections-result', (event, data) => {
            setPrimarySections(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('load-primary-sections');
            ipcRenderer.removeAllListeners('primary-section-deleted');
            ipcRenderer.removeAllListeners('search-primary-sections-result');
        };
    }, []);
    ...
    const handleConfirmDelete = () => {
        ipcRenderer.send('delete-primary-section', customerIdToDelete);
    ...
    const handleSearch = () => {
        ipcRenderer.send('search-primary-sections', searchQuery);
    };
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='pb-6 text-2xl font-bold'>区分マスタ1</div>
                <div className='flex'>
...
function PrimarySectionsIndex() {
    return (
        <Routes>
            <Route path="" element={<Index />} />
            <Route path="add" element={<PrimarySectionsAdd />} />
            <Route path="edit/:id" element={<PrimarySectionsEdit />} />
            <Route path="detail/:id" element={<PrimarySectionsDetail />} />
        </Routes>
    )
}

export default PrimarySectionsIndex;
