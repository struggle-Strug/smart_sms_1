...
import DeliveryCustomersAdd from './add';
import DeliveryCustomersEdit from './edit';
import DeliveryCustomersDetail from './detail';
...
function Index() {
    ...
    useEffect(() => {
        ipcRenderer.send('load-delivery-customers');
        ipcRenderer.on('load-delivery-customers', (event, data) => {
            setDeliveryCustomers(data);
        });

        ipcRenderer.on('delivery-customer-deleted', (event, id) => {
            setDeliveryCustomers((prevCustomers) => prevCustomers.filter(customer => customer.id !== id));
        });

        ipcRenderer.on('search-delivery-customers-result', (event, data) => {
            setDeliveryCustomers(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('load-delivery-customers');
            ipcRenderer.removeAllListeners('delivery-customer-deleted');
            ipcRenderer.removeAllListeners('search-delivery-customers-result');
        };
    }, []);
    ...
    const handleDelete = (id) => {
        if (window.confirm('本当にこの納品先を削除しますか？')) {
            ipcRenderer.send('delete-delivery-customer', id);
        }
    };

    const handleSearch = () => {
        ipcRenderer.send('search-delivery-customers', searchQuery);
    };
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='pb-6 text-2xl font-bold'>納品先</div>
                <div className='flex'>
                ...
                <table className="w-full mt-8 ">
                    <thead className=''>
                        <tr className='border-b'>
                            <th className='text-left pb-2.5'>納品先名1</th>
                            <th className='text-left pb-2.5'>納品先名2</th>
                            <th className='text-left pb-2.5'>納品先コード</th>
                    ...
                    <tbody>
                        {deliveryCustomers.map((customer) => (
                            <tr className='border-b' key={customer.id}>
                                ...
                                <td>{customer.email || <div className='border w-4'></div>}</td>
                                <td className='flex justify-center relative'>
                                    <div className='border rounded px-4 py-3 relative' onClick={(e) => toggleDropdown(customer.id)}>
...
function DeliveryCustomersIndex() {
    return (
        <Routes>
            <Route path="" element={<Index />} />
            <Route path="add" element={<DeliveryCustomersAdd />} />
            <Route path="edit/:id" element={<DeliveryCustomersEdit />} />
            <Route path="detail/:id" element={<DeliveryCustomersDetail />} />
        </Routes>
    )
}

export default DeliveryCustomersIndex;
