...
function Index() {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('search-contact-persons-result', (event, data) => {
            setContactPersons(data);
        });
        ...
        return () => {
            ...
            ipcRenderer.removeAllListeners('search-contact-persons-result');
        };
    }, []);
    ...
    const handleConfirmDelete = () => {
        ipcRenderer.send('delete-contact-person', customerIdToDelete);
    ...
    const handleSearch = () => {
        ipcRenderer.send('search-contact-persons', searchQuery);
    };
                ...
                <table className="w-full mt-8 table-auto">
                    <thead>
                        <tr className='border-b'>
                            <th className='text-left pb-2.5'>担当者名</th>
                            <th className='text-left pb-2.5'>所属部署</th>
                            <th className='text-left pb-2.5'>役職</th>
                        ...
                        {contactPersons.map((person) => (
                            <tr className='border-b' key={person.id}>
                                ...
                                <td className='flex justify-center relative'>
                                    <div className='border rounded px-4 py-3 relative' onClick={() => toggleDropdown(person.id)}>
                                    {isOpen === person.id && <DropDown id={person.id} name={person.name} />}
