...
function ShopsEdit() {
    ...
    useEffect(() => {
        // 初期ロード時に店舗データを取得
        ipcRenderer.send('edit-shop', id);
        ipcRenderer.on('edit-shop', (event, shopData) => {
            setShop(shopData);
        });

        return () => {
            ipcRenderer.removeAllListeners('edit-shop');
        };
    }, [id]);
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='text-2xl font-bold mb-8'>{shop.name}</div>
