...
import ShopsAdd from './add';
import ShopsEdit from './edit';
import ShopsDetail from './detail';
...
function Index() {
    ...
    useEffect(() => {
        ipcRenderer.send('load-shops');
        ipcRenderer.on('load-shops', (event, data) => {
            setShops(data);
        });

        ipcRenderer.on('shop-deleted', (event, id) => {
            setShops((prevShops) => prevShops.filter(shop => shop.id !== id));
        });

        ipcRenderer.on('search-shops-result', (event, data) => {
            setShops(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('load-shops');
            ipcRenderer.removeAllListeners('shop-deleted');
            ipcRenderer.removeAllListeners('search-shops-result');
        };
    }, []);
    ...
    const handleDelete = (id) => {
        if (window.confirm('本当にこの店舗を削除しますか？')) {
            ipcRenderer.send('delete-shop', id);
        }
    };

    const handleSearch = () => {
        ipcRenderer.send('search-shops', searchQuery);
    };
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='pb-6 text-2xl font-bold'>店舗一覧</div>
                <div className='flex'>
                ...
                <table className="w-full mt-8 table-auto">
                    <thead>
                        <tr className='border-b'>
                            <th className='text-left pb-2.5'>店舗名</th>
                            <th className='text-left pb-2.5'>店舗コード</th>
                            ...
                            <tr className='border-b' key={shop.id}>
                                ...
                                <td className='flex justify-center relative'>
                                    <div className='border rounded px-4 py-3 relative' onClick={() => toggleDropdown(shop.id)}>
                                        {isOpen === shop.id && <DropDown id={shop.id} />}
...
function ShopsIndex() {
    return (
        <Routes>
            ...
            <Route path="add" element={<ShopsAdd />} />
            <Route path="edit/:id" element={<ShopsEdit />} />
            <Route path="detail/:id" element={<ShopsDetail />} />
        </Routes>
    )
}
