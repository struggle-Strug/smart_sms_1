...
function Index() {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('payment-method-deleted', (event, id) => {
            setPaymentMethods((prevMethods) => prevMethods.filter(method => method.id !== id));
        });

        ipcRenderer.on('search-payment-methods-result', (event, data) => {
            setPaymentMethods(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('load-payment-methods');
            ipcRenderer.removeAllListeners('payment-method-deleted');
            ipcRenderer.removeAllListeners('search-payment-methods-result');
        };
    }, []);
    ...
    const handleSearch = () => {
        ipcRenderer.send('search-payment-methods', searchQuery);
    };
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='pb-6 text-2xl font-bold'>支払方法一覧</div>
                <div className='flex'>
                ...
                <table className="w-full mt-8 table-auto">
                    <thead>
                        <tr className='border-b'>
                            <th className='text-left pb-2.5'>支払い方法名</th>
                            <th className='text-left pb-2.5'>支払い方法コード</th>
...
function PaymentMethodsIndex() {
    return (
        <Routes>
            ...
            <Route path="add" element={<PaymentMethodsAdd />} />
            <Route path="edit/:id" element={<PaymentMethodsEdit />} />
            <Route path="detail/:id" element={<PaymentMethodsDetail />} />
        </Routes>
    )
}
