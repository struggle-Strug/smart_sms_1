...
import SecondarySectionsAdd from './add';
import SecondarySectionsEdit from './edit';
import SecondarySectionsDetail from './detail';
...
function Index() {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('secondary-section-deleted', (event, id) => {
            setSecondarySections((prevSections) => prevSections.filter(section => section.id !== id));
        });

        ipcRenderer.on('search-secondary-sections-result', (event, data) => {
            setSecondarySections(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('load-secondary-sections');
            ipcRenderer.removeAllListeners('secondary-section-deleted');
            ipcRenderer.removeAllListeners('search-secondary-sections-result');
        };
    }, []);
    ...
    const handleConfirmDelete = () => {
        ipcRenderer.send('delete-secondary-section', customerIdToDelete);
    ...
    const handleSearch = () => {
        ipcRenderer.send('search-secondary-sections', searchQuery);
    };
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='pb-6 text-2xl font-bold'>区分マスタ2</div>
                <div className='flex'>
                ...
                <table className="w-full mt-8 table-auto">
                    <thead>
                        <tr className='border-b'>
                            <th className='text-left pb-2.5'>区分名</th>
                            <th className='text-left pb-2.5'>区分コード</th>
                        ...
                        {secondarySections.map((section) => (
                            <tr className='border-b' key={section.id}>
                                ...
                                <td className='flex justify-center relative'>
                                    ...
                                    {isOpen === section.id && <DropDown id={section.id} name={section.name} />}
...
function SecondarySectionsIndex() {
    return (
        <Routes>
            <Route path="" element={<Index />} />
            <Route path="add" element={<SecondarySectionsAdd />} />
            <Route path="edit/:id" element={<SecondarySectionsEdit />} />
            <Route path="detail/:id" element={<SecondarySectionsDetail />} />
        </Routes>
    )
}

export default SecondarySectionsIndex;
