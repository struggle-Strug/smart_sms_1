...
function Index() {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('category-deleted', (event, id) => {
            setCategories((prevCategories) => prevCategories.filter(category => category.id !== id));
        });

        ipcRenderer.on('search-categories-result', (event, data) => {
            setCategories(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('load-categories');
            ipcRenderer.removeAllListeners('category-deleted');
            ipcRenderer.removeAllListeners('search-categories-result');
        };
    }, []);
    ...
    const handleConfirmDelete = () => {
        ipcRenderer.send('delete-category', categoryIdToDelete);
    ...
    const handleSearch = () => {
        ipcRenderer.send('search-categories', searchQuery);
    };
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='pb-6 text-2xl font-bold'>カテゴリー</div>
                <div className='flex'>
                ...
                <table className="w-full mt-8 table-auto">
                    <thead>
                        <tr className='border-b'>
                            <th className='text-left pb-2.5'>カテゴリー名</th>
                            <th className='text-left pb-2.5'>カテゴリーコード</th>
                    ...
                    <tbody>
                        {categories.map((category) => (
                            <tr className='border-b' key={category.id}>
                                <td>{category.name || <div className='border w-4'></div>}</td>
                                <td>{category.code || <div className='border w-4'></div>}</td>
                                <td className='flex justify-center relative'>
                                    <div className='border rounded px-4 py-3 relative' onClick={() => toggleDropdown(category.id)}>
                                    {isOpen === category.id && <DropDown id={category.id} name={category.name} />}
