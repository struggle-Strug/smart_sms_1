...
import CategoriesDetail from './detail';
...
function Index() {
    ...
    useEffect(() => {
        ipcRenderer.send('load-subcategories')
        ipcRenderer.on('load-subcategories', (event, data) => {
            setSubcategories(data);
        });

        ipcRenderer.on('subcategory-deleted', (event, id) => {
            setSubcategories((prevCategories) => prevCategories.filter(subcategory => subcategory.id !== id));
        });

        ipcRenderer.on('search-subcategories-result', (event, data) => {
            setSubcategories(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('load-subcategories');
            ipcRenderer.removeAllListeners('subcategory-deleted');
            ipcRenderer.removeAllListeners('search-subcategories-result');
        };
    }, []);
    ...
    const handleConfirmDelete = () => {
        ipcRenderer.send('delete-subcategory', subcategoryIdToDelete);
    ...
    const handleSearch = () => {
        ipcRenderer.send('search-subcategories', searchQuery);
    };
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='pb-6 text-2xl font-bold'>サブカテゴリー</div>
                <div className='flex'>
                ...
                <table className="w-full mt-8 table-auto">
                    <thead>
                        <tr className='border-b'>
                            <th className='text-left pb-2.5'>サブカテゴリー名</th>
                            <th className='text-left pb-2.5'>サブカテゴリーコード</th>
                    ...
                    <tbody>
                        {subcategories.map((subcategory) => (
                            <tr className='border-b' key={subcategory.id}>
                                <td>{subcategory.name || <div className='border w-4'></div>}</td>
                                <td>{subcategory.code || <div className='border w-4'></div>}</td>
                                <td className='flex justify-center relative'>
                                    <div className='border rounded px-4 py-3 relative' onClick={() => toggleDropdown(subcategory.id)}>
                                    {isOpen === subcategory.id && <DropDown id={subcategory.id} name={subcategory.name} />}
...
function CategoriesIndex() {
    return (
        <Routes>
            ...
            <Route path="detail/:id" element={<CategoriesDetail />} />
        </Routes>
    )
}
