...
function SubcategoriesEdit() {
    ...
    useEffect(() => {
        // 初期ロード時にカテゴリーのデータを取得
        ipcRenderer.send('edit-subcategory', id);
        ipcRenderer.on('edit-subcategory', (event, subcategoryData) => {
            setCategory(subcategoryData);
        });

        return () => {
            ipcRenderer.removeAllListeners('edit-subcategory');
        };
    }, [id]);
    ...
    return (
        <div className='w-full'>
            <div className='p-8'>
                <div className='text-2xl font-bold mb-8'>{subcategory.name}</div>
