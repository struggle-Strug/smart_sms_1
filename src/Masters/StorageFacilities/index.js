...
function StorageFacilityList() {
    ...
    const handleDelete = (id) => {
        if (window.confirm('本当にこの倉庫を削除しますか？')) {
            ipcRenderer.send('delete-storage-facility', id);
        }
    };
                        ...
                        {facilities.map((facility) => (
                            <tr className='border-b' key={facility.id}>
                                ...
                                <td className='flex justify-center relative'>
                                    {isOpen === facility.id && <DropDown id={facility.id} />}
                                    <div className='border rounded px-4 py-3' onClick={() => toggleDropdown(facility.id)}>
