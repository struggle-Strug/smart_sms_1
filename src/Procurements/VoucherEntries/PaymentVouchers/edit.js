...
function PaymentVouchersEdit() {
    ...
    const handleSubmit = () => {
        ...
        if (!validator.hasErrors()) {
            ...
            ipcRenderer.send('delete-payment-voucher-details-by-pv-id', id);
            ipcRenderer.send('delete-pos-pvs-mappings-by-pvs-id', id);
                for (let i = 0; i < paymentVoucherDetails.length; i++) {
                    const paymentVoucherDetailData = paymentVoucherDetails[i];
                    paymentVoucherDetailData.payment_voucher_id = paymentVoucher.id;
                    ipcRenderer.send('save-payment-voucher-detail', paymentVoucherDetailData);
            }

            console.log(connectedPurchaseOrders)
...
export default PaymentVouchersEdit;
