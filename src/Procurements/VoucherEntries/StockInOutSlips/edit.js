...
function StockInOutSlipsEdit() {
    ...
    const handleSubmit = () => {
        ...
        if (!validator.hasErrors()) {
            ...
            ipcRenderer.send('delete-stock-in-out-slip-details-by-slip-id', id);
            for (let i = 0; i < stockInOutSlipDetails.length; i++) {
                const stockInOutSlipDetailData = stockInOutSlipDetails[i];
                stockInOutSlipDetailData.stock_in_out_slip_id = stockInOutSlip.id;
                delete stockInOutSlipDetailData.id
