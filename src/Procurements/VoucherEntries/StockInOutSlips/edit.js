...
function StockInOutSlipsEdit() {
    ...
    useEffect(() => {
        // 初期ロード時に会社データを取得
        ipcRenderer.send('edit-stock-in-out-slip', id);
        ipcRenderer.on('edit-stock-in-out-slip', (event, stockInOutSlipsData) => {
            setStockInOutSlips(stockInOutSlipsData);
        });

        return () => {
            ipcRenderer.removeAllListeners('edit-stock-in-out-slip');
        };
    }, [id]);
    ...
    const handleSubmit = () => {
        ...
        // validator.required(stockInOutSlips.order_date, 'order_date', '発注日付');
        // validator.required(stockInOutSlips.vender_id, 'vender_id', '仕入先コード');
        // validator.required(stockInOutSlips.vender_name, 'vender_name', '仕入先名');
