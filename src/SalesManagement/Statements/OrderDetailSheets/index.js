...
function Index() {
    ...
    const fileName = `受注明細表_${year}${month}${day}_${hours}${minutes}${seconds}`;
                        ...
                        <tbody>
                            {orderSlipDetails.map((orderSlipDetail, index) => (
                                <tr className="border-b" key={index}>
                                    <td className="py-4">{orderSlipDetail.order_date || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.code || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.estimateNumber || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.vender_name || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.product_code || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.product_name || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.category || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.subcategory || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.number || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.price || '-'}</td>
                                    <td className="py-4">{parseInt(orderSlipDetail.number) * parseInt(orderSlipDetail.price) || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.gross_margin_rate || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.gross_profit || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.lot_number || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.storage_facility || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.contact_person || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.classification_primary || '-'}</td>
                                    <td className="py-4">{orderSlipDetail.classification_secondary || '-'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
