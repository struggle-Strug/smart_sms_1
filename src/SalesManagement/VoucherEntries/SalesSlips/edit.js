...
function SalesSlipsEdit() {
    ...
    const handleSubmit = () => {
        ...
        if (!validator.hasErrors()) {
            ...
            ipcRenderer.send('delete-sales-slip-details-by-slip-id', id);

            for (let i = 0; i < salesSlipDetails.length; i++) {
                const salesSlipDetailData = salesSlipDetails[i];
                salesSlipDetailData.sales_slip_id = salesSlip.id;
        ...
        <div className='w-full'>
            <div className=''>
                <div className='pt-8 pb-6 flex border-b px-8 items-center'>
                    <div className='text-2xl font-bold'>{'売上伝票の編集'}</div>
                ...
                <div className='px-8 py-6'>
                    ...
                    <div className='pb-2'>
                        ...
                        <DatePicker
                            selected={salesSlip.sales_date ? new Date(salesSlip.sales_date) : null}
                            onChange={(date) => handleDateChange(date, "sales_date")}
                            dateFormat="yyyy-MM-dd"
                            className='border rounded px-4 py-2.5 bg-white  w-[480px]'
                            placeholderText='売上日付を選択'
                        />
                    ...
                    <div className='pb-2'>
                        ...
                        <DatePicker
                            selected={salesSlip.delivery_due_date ? new Date(salesSlip.delivery_due_date) : null}
                            onChange={(date) => handleDateChange(date, "delivery_due_date")}
                            dateFormat="yyyy-MM-dd"
                            className='border rounded px-4 py-2.5 bg-white  w-[480px]'
                            placeholderText='納品期限を選択'
                        />
                    </div>
                    <div className='py-3'>
                        <hr className='' />
                    ...
                    <div className='pb-2'>
                        ...
                        <DatePicker
                            selected={salesSlip.closing_date ? new Date(salesSlip.closing_date) : null}
                    ...
                    <div className='pb-2'>
                        ...
                        <DatePicker
                            selected={salesSlip.deposit_due_date ? new Date(salesSlip.deposit_due_date) : null}
                            onChange={(date) => handleDateChange(date, "deposit_due_date")}
                            dateFormat="yyyy-MM-dd"
                            className='border rounded px-4 py-2.5 bg-white  w-[480px]'
                            placeholderText='入金期日を選択'
                        />
                    </div>                    <div className='pb-2 mb-24'>
