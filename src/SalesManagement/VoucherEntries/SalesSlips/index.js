...
import SalesSlipsAdd from './add';
import SalesSlipsEdit from './edit';
import SalesSlipsDetail from './detail';
...
function Index() {
  ...
  useEffect(() => {
    ipcRenderer.send('load-sales-slips', pages);
  }, [pages]);


  useEffect(() => {
    ipcRenderer.send('load-sales-slips');
    ipcRenderer.on('load-sales-slips', (event, data) => {
      setSalesSlips(data);
    });

    ipcRenderer.on('sales-slip-deleted', (event, id) => {
      setSalesSlips((prevSaless) => prevSaless.filter(sales => sales.id !== id));
    });

    ipcRenderer.on('search-sales-slips-result', (event, data) => {
      setSalesSlips(data);
    });

    return () => {
      ipcRenderer.removeAllListeners('sales-slips-data');
      ipcRenderer.removeAllListeners('search-sales-slips-result');
    };
  }, []);
  ...
  const handleSearch = () => {
    ipcRenderer.send('search-sales-slips', searchQuery);
  };
  ...
  return (
    <div className='w-full'>
      <div className='p-8'>
        <div className='pb-6 text-2xl font-bold'>売上伝票</div>
        <div className='flex'>
          ...
          <tbody>
            {salesSlips.map((slip) => (
              <tr className='border-b' key={slip.id}>
                <td>{slip.sales_date || <div className='border w-4'></div>}</td>
...
function SalesSlipsIndex() {
  return (
    <Routes>
      <Route path="" element={<Index />} />
      <Route path="add" element={<SalesSlipsAdd />} />
      <Route path="edit/:id" element={<SalesSlipsEdit />} />
      <Route path="detail/:id" element={<SalesSlipsDetail />} />
    </Routes>
  )
}

export default SalesSlipsIndex;
