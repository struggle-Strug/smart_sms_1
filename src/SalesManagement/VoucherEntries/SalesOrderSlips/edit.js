...
function SalesOrderSlipsEdit() {
    ...
    const handleSubmit = () => {
        ...
        if (!validator.hasErrors()) {
            ...
            console.log("order-slip saved:", orderSlip);
            ipcRenderer.send('delete-order-slip-details-by-slip-id', id);
            for (let i = 0; i < orderSlipDetails.length; i++) {
                const orderSlipDetailData = orderSlipDetails[i]
                orderSlipDetailData.order_slip_id = orderSlip.id;
        ...
        <div className='w-full'>
            <div className=''>
                <div className='pt-8 pb-6 flex border-b px-8 items-center'>
                    <div className='text-2xl font-bold'>{'受注伝票の編集'}</div>
                ...
                <div className='px-8 py-6'>
                    ...
                    <div className='pb-2'>
                        ...
                        <DatePicker
                            selected={orderSlip.order_date ? new Date(orderSlip.order_date) : null}
                    ...
                    <a data-tooltip-id="my-tooltip" data-tooltip-content="納品日付ツールチップ 納品日付までの期間、在庫予定数が減り、在庫マスタ警告値に在庫予定数が到達すると在庫補充アラートが出る。" className='flex ml-3'>
                        ...
                        <DatePicker
                            selected={orderSlip.delivery_date ? new Date(orderSlip.delivery_date) : null}
                    ...
                    <div className='pb-2'>
                        ...
                        <DatePicker
                            selected={orderSlip.closing_date ? new Date(orderSlip.closing_date) : null}
                    ...
                    <div className='pb-2'>
                        ...
                        <DatePicker
                            selected={orderSlip.deposit_due_date ? new Date(orderSlip.deposit_due_date) : null}
            ...
            <div className='flex mt-8 fixed bottom-0 border-t w-full py-4 px-8 bg-white'>
                ...
                <Link to={`salesmanagements/order-slip`} className='border rounded px-4 py-3 font-bold cursor-pointer'>キャンセル</Link>
            </div>
        </div>
    );
}
...
export default SalesOrderSlipsEdit;
