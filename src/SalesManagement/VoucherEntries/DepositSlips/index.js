...
import DepositSlipsAdd from './add';
import DepositSlipsDetail from './detail';
import DepositSlipsEdit from './edit';
...
function Index() {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('load-deposit-slips', (event, data) => {
            setDepositSlips(data);
        });

        ipcRenderer.on('deposit-slip-deleted', (event, id) => {
            setDepositSlips((prevSaless) => prevSaless.filter(sales => sales.id !== id));
        });

        ipcRenderer.on('search-deposit-slips-result', (event, data) => {
            setDepositSlips(data);
        });

        return () => {
            ipcRenderer.removeAllListeners('deposit-slips-data');
            ipcRenderer.removeAllListeners('search-deposit-slips-result');
        };
    }, []);
...
const handleSearch = () => {
    ipcRenderer.send('search-deposit-slips', searchQuery);
};
                    ...
                    <tbody>
                        {depositSlips.map((slip) => (
                            <tr className='border-b' key={slip.id}>
                                <td>{slip.deposit_date || <div className='border w-4'></div>}</td>
                                ...
                                <td>{slip.remarks || <div className='border w-4'></div>}</td>
...
function DepositSlipsIndex() {
    return (
        <Routes>
            <Route path="" element={<Index />} />
            <Route path="add" element={<DepositSlipsAdd />} />
            <Route path="detail/:id" element={<DepositSlipsDetail />} />
            <Route path="edit/:id" element={<DepositSlipsEdit />} />
        </Routes>
    )
}

export default DepositSlipsIndex;
